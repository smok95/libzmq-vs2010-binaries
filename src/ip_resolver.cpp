...
// 2020.07.09 kim,jk
// if_indextoname 및 if_nametoindex 동적로드 방식으로 변경 (Windows XP에서 오류발생에 따른 수정)
// dll정적링크 오류만 발생하지 않도록 수정한 것이며, XP에서는 NULL만 리턴함.
static PCHAR WINAPI if_indextoname_safe(__in NET_IFINDEX InterfaceIndex, __out_ecount(IF_NAMESIZE) PCHAR InterfaceName) {
	typedef PCHAR (WINAPI *fn_if_indextoname)(NET_IFINDEX InterfaceIndex, PCHAR InterfaceName);
	PCHAR ret = NULL;
	if(HMODULE hDll = LoadLibraryA("Iphlpapi.dll")) {
		fn_if_indextoname _if_indextoname = (fn_if_indextoname)GetProcAddress(hDll, "if_indextoname");
		if(_if_indextoname)
...
static NET_IFINDEX WINAPI if_nametoindex_safe(__in PCSTR InterfaceName) {
	typedef NET_IFINDEX (WINAPI *fn_if_nametoindex)(PCSTR InterfaceName);
	NET_IFINDEX ret = 0;
	if(HMODULE hDll = LoadLibraryA("Iphlpapi.dll")) {
		fn_if_nametoindex _if_nametoindex = (fn_if_nametoindex)GetProcAddress(hDll, "if_nametoindex");
		if(_if_nametoindex)
			ret = _if_nametoindex(InterfaceName);
		FreeLibrary(hDll);
	}
	return ret;
}
                                            ...
                                            char **dest_) const
{
    ...
    if_name_result = if_indextoname_safe(index_, buffer);
#endif
...
unsigned int zmq::ip_resolver_t::do_if_nametoindex (const char *ifname_)
{
  ...
  && !defined ZMQ_HAVE_VXWORKS
    return if_nametoindex_safe (ifname_);
#else
